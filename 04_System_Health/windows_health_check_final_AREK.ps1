<#
windows_health_check_final_AREK.ps1
Poprawiona wersja skryptu NyX - bez lania wody i zbędnych błędów.
Optymalizacja: Timeouty, bezpieczniejszy skan plików i odporność na wyłączonego Defendera.
#>
...
param(
    ...
    [int]$JobTimeout = 15,                 # Zwiększony timeout dla stabilności
...
function SafeJob {
    ...
    try {
        ...
        if (Wait-Job -Job $job -Timeout $TimeoutSeconds) {
            ...
            Remove-Job -Job $job -Force -ErrorAction SilentlyContinue
            return [PSCustomObject]@{ Status = "Timeout po $TimeoutSeconds s" }
        }
    } catch {
...
function Section-LargestFiles {
    param($Path="C:\Users",$MinMB=500) # Zmiana na Users i 500MB dla szybkości
    try {
        Write-Log "Skanowanie dużych plików w $Path (Min: $MinMB MB)..."
        return Get-ChildItem -Path $Path -File -Recurse -ErrorAction SilentlyContinue -Force |
...
function Section-Defender {
    try {
        ...
        return [PSCustomObject]@{ Note = "Defender niedostępny lub usługa wyłączona" }
...
foreach ($s in $Sections) {
    ...
    try {
        switch ($s) {
            ...
            "LargestFiles"    { $report.Results.LargestFiles = Section-LargestFiles }
...
try {
    if ($report.Results.Defender -and ($report.Results.Defender.AVRunning -ne $true -and $report.Results.Defender.AMRunning -ne $true)) {
        $recs += [PSCustomObject]@{ Item="Antywirus"; Issue="Defender nieaktywny"; Action="Sprawdź stan usługi Security Center"; Risk="High" }
    }
} catch {}

# --- Zapis ---
$report | ConvertTo-Json -Depth 8 | Out-File -FilePath $OutJson -Encoding UTF8
Write-Log "Raport JSON gotowy: $OutJson"

$html = "<html><head><meta charset='utf-8'><title>Health Report</title><style>body{font-family:sans-serif;background:#1a1a1a;color:#eee}pre{background:#2d2d2d;padding:15px;border-radius:5px}</style></head><body><h1>Health Report - $env:COMPUTERNAME</h1><pre>$($report | Out-String)</pre></body></html>"
$html | Out-File -FilePath $OutHtml -Encoding UTF8

$recs | ConvertTo-Json -Depth 6 | Out-File -FilePath $OutRec -Encoding UTF8
Write-Log "Rekomendacje zapisane w $OutRec. Robota skończona."
